...
def run_ps(cmd):
    ...
    return run_shell(
        cmd, shell="powershell", switch="-Command", path="(pwd).Path", useShell=False
    )
...
def run_shell(cmd, shell, switch, path, useShell):
    ...
    try:
        ...
        cmd_pwd = f"({cmd}) ; {path}"
        cmd_pwd_output = subprocess.run(
            [shell, switch, cmd_pwd], capture_output=True, text=True, shell=useShell
        )
