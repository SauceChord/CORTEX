...
class SettingsResponse(BaseModel):
    ...
    shell: Optional[Literal["powershell", "bash"]] = Field(
        None, description="The users chosen shell terminal."
    )
    model: Optional[
        Literal[
            "gpt-4o",
    ...
    ] = Field(None, description="The AI model engine that Cortex will use.")
    explain: Optional[bool] = Field(
        None,
        description="If set to True, Cortex will explain commands after execution. If set to False, less usage cost and faster response times.",
    )
    autocomplete: Optional[bool] = Field(
        None,
        description="If set to true, autocompletes with files found in the current users directory. Only used in RUN mode.",
    )


class RequestResponse(BaseModel):
    message: str = Field(..., description="A response to the users message")
    settings: Optional[SettingsResponse] = Field(
        None,
        description="Populated when the user wants to change any of the settings related to Cortex.",
    )
